...
(-y) --symbol_server_output | Use implicit symbol-server output path with lowercase GUID+Age: <output>/<input-file-name>/<guidage>/<input-file-name> when using --compress.
(-Y) --symbol_server_output_uppercase | Use implicit symbol-server output path with uppercase GUID+Age: <output>/<input-file-name>/<GUIDAGE>/<input-file-name> when using --compress.
```
...
- (optional) **-\-symbol_server_output** / **-\-symbol_server_output_uppercase**, if we want the output path to follow symbol server layout. When either option is used, `--output` is treated as the root directory and the final output path becomes `<output>/<input-file-name>/<GUIDAGE>/<input-file-name>`. The lowercase variant (`-y`) emits a lowercase directory name (e.g. `symbols/game.pdb/00112233445566778899aabbccddeeff1/game.pdb`), while the uppercase variant (`-Y`) emits an uppercase one (e.g. `symbols/game.pdb/00112233445566778899AABBCCDDEEFF1/game.pdb`). The two options are mutually exclusive.
...
- During **--compress** and **--decompress**, pdbconv prints the detected input PDB GUID from the PDB info stream.
